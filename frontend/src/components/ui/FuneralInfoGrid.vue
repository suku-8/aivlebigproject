    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="createObituary" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>부고장생성
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="createDeathReport" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>사망신고서생성
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="createSchedule" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>장례일정표생성
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="createAllDocuments" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>서류일괄생성
                </v-btn>
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="updateFuneralInfo" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>장례정보수정
                </v-btn>
            </div>
...
export default {
    ...
    methods:{
        async createObituary(){
            try{
                var path = "createObituary".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','createObituary 성공적으로 처리되었습니다.')
        ...
        async createDeathReport(){
            try{
                var path = "createDeathReport".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','createDeathReport 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
        ...
        async createSchedule(){
            try{
                var path = "createSchedule".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','createSchedule 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
        ...
        async createAllDocuments(){
            try{
                var path = "createAllDocuments".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','createAllDocuments 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
        ...
        async updateFuneralInfo(){
            try{
                var path = "updateFuneralInfo".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','updateFuneralInfo 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
